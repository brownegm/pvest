test_that("validate inputs", {
  ...
  expect_error(estPV(data = pvest::quag, species, leaf, fresh.rain, water.potential, hot.weight))
  
  #inspect output
  default <- estPV(pvest::quag, species, subgrp = leaf, fw = fresh.weight, water.potential, dry.weight)
  ...
  expect_equal(length(default), length(unique(paste0(pvest::quag$species, pvest::quag$leaf))))
