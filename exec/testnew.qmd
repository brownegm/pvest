...
df <- data.frame(r = srwc, psi_w = quag1$water.potential)
...
fit <- nlsLM(
  ...
  start = list(
              r_tlp = median(srwc), 
               b = 8),          # Starting guess for RWC at TLP # Starting guess for elasticity exponent
  lower = c(0.6, 4),            # Lower bounds (physiological constraints)
  upper = c(0.9, 10),              # Upper bounds
  control = nls.lm.control(maxiter = 50)  # Increase iterations for stability
)
