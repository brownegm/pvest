...
uniques <- readxl::read_xlsx(
  here("inst", "extdata", "summary_main.xlsx"),
  sheet = "unique_ids"
) %>%
  pull(unique_id)
...
pred_vars_index <- which(names(com) %in% pred_vars)[c(
  1,
  2,
  6,
  3:5,
...
pv_lms <- purrr::map2(
  # compute linear models for each variable pair
  c(9:20),
  pred_vars_index,
...
pv_lms.origin <- purrr::map2(
  # compute linear models for each variable pair forced through origin
  c(9:20),
  pred_vars_index,
...
pred_vars_index.sp <- which(names(com_species) %in% pred_vars)[c(
  1,
  2,
  6,
  3:5,
...
# estimate linear models and summaries
pv_lms.sp <- purrr::map2(
  # compute linear models for each variable pair
  c(2:13),
  pred_vars_index.sp,
  ~ lm(
    as.formula(paste(names(com_species)[.x], "~", names(com_species)[.y])),
    data = com_species
  )
)
...
pv_lms.origin.sp <- purrr::map2(
  # compute linear models for each variable pair fitted through the origin
  c(2:13),
  pred_vars_index.sp,
  ~ lm(
    as.formula(paste(
...
# prep for prediction intervals
pv_sp <- map(pred_vars_index.sp, \(coln) {
  f(data = com_species, names(com_species)[coln])
}) # use function f to create "newdata" dataframes for each variable
...
text(
  x = 1:length(cooks.d) + 1,
  y = cooks.d,
  labels = ifelse(cooks.d > 4 * mean(cooks.d, na.rm = T), names(cooks.d), ""),
  col = "red"
) # add labels

## find the influential values
cooks.d_odd <- as.numeric(names(cooks.d)[
  (cooks.d > 4 * mean(cooks.d, na.rm = T))
...
plot(
  coms_pio.odd$osm.pot.fullturgor,
  coms_pio.odd$pi_o,
  pch = 21,
  cex = 2,
  col = "black",
...
plot(
  com$swc ~ com$saturated.water.content,
  pch = 21,
  cex = 2,
  col = "black",
  bg = "#4f8359"
...
legend(
  "bottomright",
  legend = c("1:1", "OLS", "Pred.Int"),
  lty = c(1, 1, 2),
  col = c("black", "#4f8359", "orangered"),
  lwd = 2,
  bty = "n"
)
...
plot(
  com$relative.water.content.attlp,
  com$rwc_tlp,
  pch = 21,
  cex = 2,
  col = "black",
...
plot(
  com_species$swc ~ com_species$saturated.water.content,
  pch = 21,
  cex = 2,
  col = "black",
  bg = "#4f8359"
...
lines(
  pv_sp[[1]][[1]],
  pv_pred_intervals.origin.sp[[1]][[3]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # upper
lines(
  pv_sp[[1]][[1]],
  pv_pred_intervals.origin.sp[[1]][[2]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # lower
...
plot(
  com_species$osm.pot.fullturgor,
  com_species$pi_o,
  pch = 21,
  cex = 2,
  col = "black",
...
lines(
  pv_sp[[2]][[1]],
  pv_pred_intervals.origin.sp[[2]][[3]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # upper
lines(
  pv_sp[[2]][[1]],
  pv_pred_intervals.origin.sp[[2]][[2]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # lower
...
plot(
  com_species$leaf.waterpotential.attlp,
  com_species$psi_tlp,
  pch = 21,
  cex = 2,
  col = "black",
...
lines(
  pv_sp[[3]][[1]],
  pv_pred_intervals.origin.sp[[3]][[3]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # upper
lines(
  pv_sp[[3]][[1]],
  pv_pred_intervals.origin.sp[[3]][[2]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # lower
...
plot(
  com_species$apoplastic.fraction,
  com_species$af,
  pch = 21,
  cex = 2,
  col = "black",
...
lines(
  pv_sp[[4]][[1]],
  pv_pred_intervals.origin.sp[[4]][[3]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # upper
lines(
  pv_sp[[4]][[1]],
  pv_pred_intervals.origin.sp[[4]][[2]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # lower
...
plot(
  com_species$relative.water.content.attlp,
  com_species$rwc_tlp,
  pch = 21,
  cex = 2,
  col = "black",
...
lines(
  pv_sp[[5]][[1]],
  pv_pred_intervals.origin.sp[[5]][[3]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # upper
lines(
  pv_sp[[5]][[1]],
  pv_pred_intervals.origin.sp[[5]][[2]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # lower
...
lines(
  pv_sp[[7]][[1]],
  pv_pred_intervals.origin.sp[[7]][[3]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # upper
lines(
  pv_sp[[7]][[1]],
  pv_pred_intervals.origin.sp[[7]][[2]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # lower
...
lines(
  pv_sp[[8]][[1]],
  pv_pred_intervals.origin.sp[[8]][[3]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # upper
lines(
  pv_sp[[8]][[1]],
  pv_pred_intervals.origin.sp[[8]][[2]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # lower
...
lines(
  pv_sp[[9]][[1]],
  pv_pred_intervals.origin.sp[[9]][[3]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # upper
lines(
  pv_sp[[9]][[1]],
  pv_pred_intervals.origin.sp[[9]][[2]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # lower
...
lines(
  pv_sp[[10]][[1]],
  pv_pred_intervals.origin.sp[[10]][[3]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # upper
lines(
  pv_sp[[10]][[1]],
  pv_pred_intervals.origin.sp[[10]][[2]],
  col = "orangered",
  lty = "dashed",
  lwd = 2
) # lower
...
write.csv(
  pv_params_byleaf,
  here("inst/extdata", "leafparams_sum.csv"),
  row.names = F
)
