...
plotPV <- function(obj, ...) {
  ...
  breakpoint <- model_result$breakpoint
  segment1 <- model_result$segment1  # Contains intercept and slope
  segment2 <- model_result$segment2  # Contains intercept and slope
  
  # Create data for the two line segments
  ...
  line_data <- data.frame(
    ...
    segment = factor(c(rep("before", length(x_before)), 
                       rep("after", length(x_after))))
  )
  
  # Create and return the plot
  ggplot() +
    geom_line(data = line_data, aes(x = x, y = y, color = segment)) +
...
StatPV <- ggplot2::ggproto(
  ...
  compute_group = function(data, scales, model_fun, attr_name =
    ...
    # list(
    #   breakpoint = value,
    #   segment1 = list(intercept = value, slope = value),
    #   segment2 = list(intercept = value, slope = value)
    # )
    
    breakpoint <- line_params$breakpoint
    segment1 <- line_params$segment1
    segment2 <- line_params$segment2
